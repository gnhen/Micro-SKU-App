...
import { getRemoteState } from './remoteState';
...
export const checkForUpdates = async (silentOnUpToDate = false) => {
  ...
  try {
    const { state, release } = await getRemoteState();

    if (state !== 'active' || !release || !release.tag_name) {
      // Unavailable, offline, or no release info: nothing to compare against.
      return null;
    }

    const latestVersion = release.tag_name; // e.g., "v0.1.2"
